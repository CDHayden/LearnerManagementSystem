...
* https://git-scm.com/docs/git-add - Helped with manually editing git hunks.

* https://stackoverflow.com/questions/10542493/using-flask-blueprints-how-to-fix-url-for-from-breaking-if-a-subdomain-is-speci - Helped with using the url_for function with blueprint endpoints

* I used the pymongo and mongodb docs a lot during this project, however I wanted  to specifically highlight the following:
	* https://docs.mongodb.com/manual/reference/operator/update/positional-filtered/ - Helped me to use postitional identifiers and array filters in my mongo queries

	* https://pymongo.readthedocs.io/en/stable/examples/aggregation.html - Helped with my aggregation queries
...
Mongomock does not support the use of the named parameter "array_filters" which was used in the add_course method on the user_model. I have tested the functionality on my local database and it works, but this means the unit test test_can_edit_existing_subject_course in user_model_test.py will not work; so I have marked it to be skipped.

To run tests nagivate to the project's root directory and run the command: `python -m unittest discover -p "*_test.py"`

## Things to add in future

Make the Add / Edit / Delete student functions better on the Staff > My Classes. Currently when one of these actions is performed, the user must then reselect the subject in the menu.

The way the menu is generated is unsorted. This means that sometimes classes can change order (for example, one time being listening > speaking and another speaking > listening). This can be jarring to the user as their learned experience of the page is not consistent. Unfortunately, given my time limitations in getting this project submitted and the complexity of the mongo query involved I did not manage to fix this presently.
